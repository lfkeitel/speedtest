...
function drawMeter(c, amount, bk, fg, progress, prog) {
  ...
  // Draw colored part that shows amount
  ctx.beginPath();
  ctx.strokeStyle = fg;
  ctx.lineWidth = 16 * sizScale;
...
function startStop() {
  if (w != null) {
    ...
    I('status').classList.remove('running');
    I('status').textContent = 'Aborted';
  } else {
    ...
    I('status').classList.remove('done');
    I('status').classList.add('running');
    ...
    w.onmessage = function(e) {
      ...
      if (status >= 4) {
        // test completed
        I('startStopBtn').classList.remove('running');
        I('status').classList.add('done');
        I('status').classList.remove('running');
        I('status').textContent = 'Done';
        w = null;
        updateUI(true);
      }
    };
